...
class Core(object):
  ...
  def get(self, datapath, start_coord, vol_size, segmentation=False, segcolor=False, fit=False, w=0):
    ...
    for z in range(vol_size[2]):
      ...
      x_tiles = range((start_coord[0] // blocksize[0]),
                      (((start_coord[0] + vol_size[0] - 1) // blocksize[0]) + 1))
      y_tiles = range((start_coord[1] // blocksize[1]),
      ...
      for x in x_tiles:
        ...
        for y in y_tiles:
          ...
          target_boundaries = (self.vol_xy_start[1], self.vol_xy_start[1] + y_offset, self.vol_xy_start[0],
                               self.vol_xy_start[0] + x_offset, z)
          source_boundaries = (self.tile_xy_start[1], self.tile_xy_start[1] + y_offset, self.tile_xy_start[0],
          ...
          if color_img:
            data = cur_img[source_boundaries[0]:source_boundaries[1],
                   source_boundaries[2]:source_boundaries[3], :]
            vol[target_boundaries[0]:target_boundaries[1], target_boundaries[2]:target_boundaries[3],
